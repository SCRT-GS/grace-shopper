...
class Checkout extends Component {
  ...
  handleCheckoutSubmit(evt) {
    evt.preventDefault()
    this.props.submitCheckout(this.state.line1, this.state.line2, this.state.city, this.state.state, this.state.zip)
    this.setState({
      redirect: true
    })
  }
  ...
  return (
    <div id="checkout-page-container">
      ...
      <form onSubmit={this.handleCheckoutSubmit} className="checkout-form" id="checkout-form">
        <div>
          <input value={this.state.email} onChange={this.inputEmail} type="text" placeholder="Email" className="form-one-col" />
        </div>
        <div>
...
return {
    submitCheckout: (line1, line2, city, state, zip) => {
      dispatch(addNewAddress(line1, line2, city, state, zip))
    }
  }
}
